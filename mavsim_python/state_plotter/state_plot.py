...
class StatePlot():
    def __init__(self, plotbox, args):
        ...
        for name in args.state_names:
            self.add_state(name, self.sigma_bounds, args.max_length,
                           args.is_angle, args.rad2deg)
    ...
    def add_state(self,
                  ...
                  is_angle=False,
                  rad2deg=False):
        self.states[name] = StateData(sigma_bounds=sigma_bounds,
        ...
        if self.dimension == 1:
            ...
            if self.has_sigma:
                for bound in self.sigma_bounds:
                    ...
                    self.sigma_plots[bound]['lower_plot'].setData(
                        x_data,
                        state_objs[0].sigma_data[bound]['lower'],
                        pen=pen)
                    self.sigma_plots[bound]['upper_plot'].setData(
                        x_data,
                        state_objs[0].sigma_data[bound]['upper'],
                        pen=pen)
            ...
            if self.has_sigma:
                for bound in self.sigma_bounds:
                    ...
                    radius = [
                        bound * state_objs[i].get_current_sigma()
                        for i in (0, 1)
                    ]
                    ellipse = self._get_ellipse(center=center, radius=radius)
                    self.sigma_plots[bound]['plot'].setData(ellipse[0],
                                                            ellipse[1],
                                                            pen=pen)
        else:
            raise NotImplementedError(
                'Plots with dimension > 2 are not yet supported.')

        # Update the data for the plot (and marker, if necessary)
        if not self.connect:
            self.plot.setData(x_data,
                              y_data,
                              pen=None,
                              symbol=self.symbol,
            ...
            if self.marker is not None and len(x_data) > 0 and len(y_data) > 0:
                ...
                scale = self.marker_scale * min(x_range[1] - x_range[0],
                                                y_range[1] - y_range[0])
